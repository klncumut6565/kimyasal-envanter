...
def _embed_qr_code(ws, hedef_sutun: int = None, hedef_satir: int = 1,
                   genislik: int = 90, yukseklik: int = 90):
    """QR kodu Excel sayfasının sağ üst köşesine gömer.

    hedef_sutun: Varsayılan olarak son dolu sütunun 2 sağı kullanılır
                 (diğer içeriklerle örtüşmez). İsteğe bağlı elle verilebilir.
    genislik/yukseklik: Piksel cinsinden resim boyutu (Excel'de yaklaşıktır).
    """
    if not os.path.exists(QR_KOD_PATH):
        return  # dosya yoksa sessizce atla
    if hedef_sutun is None:
        hedef_sutun = (ws.max_column or 1) + 2  # son sütunun 2 sağı
    hedef_adres = get_column_letter(hedef_sutun) + str(hedef_satir)
    img = XLImage(QR_KOD_PATH)
    img.width = genislik
    img.height = yukseklik
    ws.add_image(img, hedef_adres)
...
def add_products(envanter_path: str, output_path: str, urunler: list,
    ...
    for i, urun in enumerate(urunler):
        ...
        ws.row_dimensions[target_row].height = 110
    ...
    # ── QR Kod — sağ üst köşeye (A1'den 4 sütun sağ) gömülür ──────────
    _embed_qr_code(ws)
